 ...
 * @since 0.0.3
 */
open class SingletonHolder<out O : Any, in I : Any>(private val creator: I.() -> O) {

    private val instance = atomic<O?>(null)

    fun getInstance(arg: I): O {
        instance.value?.let { return it }
        return creator(arg).also {
            instance.compareAndSet(null, it)
        }
    }
